...
for i in range(-5,5) :
    ...
    for j in range(e):
        ...
        cost= (1 / (2 * n)) * sum((alti_tr - alti_pred) ** 2) + lam * (abs(v1) + abs(v2)+abs(v3))
        if (v1 >= 0):
            D_v1 = (-1 / n) * sum(longi_tr * (alti_tr - alti_pred)) + lam
        else:
            D_v1 = (-1 / n) * sum(longi_tr* (alti_tr- alti_pred)) - lam
        if (v2 >= 0):
            D_v2 = (-1 / n) * sum(lati_tr * (alti_tr - alti_pred)) + lam
        else:
            D_v2 = (-1 / n) * sum(lati_tr * (alti_tr - alti_pred)) - lam
        if (v3 >= 0):
            D_v3 = (-1 / n) * sum(alti_tr - alti_pred) +lam
        else:
            D_c = (-1 / n) * sum(alti_tr - alti_pred) - lam
        v1 = v1 - learn * D_v1
        v2 = v2 - learn * D_v2
        v3 = v3 - learn * D_v3
        alti_pred = v1 * longi_tr + v2 * lati_tr + v3
    ...
    err = (1 / (2 * m)) * sum((alti_te - alti_pred) ** 2) + lam * (abs(v1) + abs(v2) + abs(v3))
    if it==0 :
        err_prev = err
        err_pres = err
        val_err[it]=err
